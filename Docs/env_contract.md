...
> Delivery baseline status: ApiShip/Gorgo via `@gorgo/medusa-fulfillment-apiship` is the current fresh-template delivery baseline. Direct `/store/apiship/*` is the canonical Store API contract. Live external shipment execution is default-off through `APISHIP_SHIPMENT_EXECUTION_ENABLED=false` and requires the exact opt-in value `true`.

> Статус документа: рабочая спецификация окружения по состоянию на `2026-05-11`; staging Docker/Caddy/Payload runtime using production-mode builds and manual GitHub `Deploy Staging` now exists. Real production is not provisioned yet; for staging env placeholders use [`.env.staging.example`](../.env.staging.example).
>
...
- `UNISENDER_API_KEY` и optional `UNISENDER_BASE_URL` — строго **opt-in** backend-only keys для текущего production-like email path; пустое значение не должно ломать startup, build и runtime, а при `NOTIFICATION_EMAIL_PROVIDER=unisender` система должна безопасно падать обратно на local provider;
...
- `NOTIFICATION_DEFAULT_COUNTRY_CODE` (default `ru`) определяет country segment в transactional storefront URLs email verification и password reset workflow ([`sendEmailVerificationWorkflow`](../medusa-agency-boilerplate/src/workflows/send-email-verification.ts:1), [`sendPasswordResetWorkflow`](../medusa-agency-boilerplate/src/workflows/send-password-reset.ts:1), [`sendMarketingConfirmationWorkflow`](../medusa-agency-boilerplate/src/workflows/send-marketing-confirmation.ts:1)). Приоритет: сначала явный `countryCode` из входного payload workflow (передаётся storefront route, admin resend route или Phase 1 subscriber), затем env `NOTIFICATION_DEFAULT_COUNTRY_CODE`, затем `ru` как safe fallback. Без этого дефолта `/account/verify-email?token=...` строился без `/ru/` сегмента и ловил 404 на Next.js storefront routing (`[countryCode]/(main)/account/...`).
...
- fresh template delivery startup uses ApiShip/Gorgo through `@gorgo/medusa-fulfillment-apiship`;
...
- backend startup must not require obsolete provider credentials, mode flags, or provider-specific checkout activation flags;
...
- VK redirect URI now resolves к storefront proxy [`route.ts`](../medusa-agency-boilerplate-storefront/src/app/api/auth/vk-id/callback/route.ts), а не к bare Medusa Store API path: Medusa framework применяет `ensurePublishableApiKeyMiddleware` ко всему `/store/*` namespace без per-route exemption (см. [`ensure-publishable-api-key.js`](../medusa-agency-boilerplate/node_modules/@medusajs/framework/dist/http/middlewares/ensure-publishable-api-key.js:5)), поэтому browser direct redirect от VK без `x-publishable-api-key` header возвращал `not_allowed`. Storefront proxy attaches the publishable key on the server side и forwards в backend internally, сохраняя existing signed-state CSRF guard и `Set-Cookie: _medusa_jwt` semantics; pure helpers и unit tests для proxy live в [`vk-id-callback-proxy.ts`](../medusa-agency-boilerplate-storefront/src/lib/util/vk-id-callback-proxy.ts) и [`vk-id-callback-proxy.spec.ts`](../medusa-agency-boilerplate-storefront/src/lib/util/vk-id-callback-proxy.spec.ts);
...
#### Implemented `VK ID v1.5 / Phase 5.5` no-email registration + onboarding contract

Поверх Phase 5.x (login + link) Phase 5.5 добавил регистрацию через VK ID без email. Полный контракт — в [`Docs/vk-onboarding-spec.md`](./vk-onboarding-spec.md). Env/runtime итог:

- дефолт `DEFAULT_VK_ID_SCOPES` поднят до `"vkid.personal_info phone"` в [`vk-id.ts`](../medusa-agency-boilerplate/src/modules/vk-id.ts:21); env шаблоны [`.env.example`](../.env.example) и [`.env.staging.example`](../.env.staging.example) теперь содержат `VK_ID_SCOPES="vkid.personal_info phone"` как baseline. Operator может оставить пустым/переопределить;
- добавлен operator-facing flag `VK_ID_ALLOW_NO_EMAIL_REGISTER=true` в [`.env.example`](../.env.example) и [`.env.staging.example`](../.env.staging.example) как documentation hook. Фактический gate в callback включает placeholder branch всегда (после Phase 5.5); флаг сохранён для будущего отката без миграции env;
- `VK_ID_REQUIRE_EMAIL` остаётся deprecated и runtime его игнорирует;
- backend регистрация без email материализована через детерминированный placeholder email `vk_{vk_user_id}@placeholder.internal` ([`generatePlaceholderEmail`](../medusa-agency-boilerplate/src/modules/vk-id.ts:40), [`PLACEHOLDER_EMAIL_DOMAIN`](../medusa-agency-boilerplate/src/modules/vk-id.ts:33)); detector [`isPlaceholderEmail`](../medusa-agency-boilerplate/src/modules/vk-id.ts:48) используется в guard-точках workflow и в checkout middleware;
- `customer.metadata.onboarding` ставится при регистрации через [`buildOnboardingMetadata`](../medusa-agency-boilerplate/src/modules/vk-id.ts:57): `{ status: "pending"|"complete", missing_fields, placeholder_email, vk_phone_verified, created_at, completed_at? }`. Status — строка `"complete"`, не `"completed"`. Phone попадает в `missing_fields` информационно, но не блокирует переход в `"complete"`: onboarding endpoint всегда фильтрует `"phone"` из required set;
- `customer.metadata.vk_link` обогащён полями `phone`, `avatar`, `phone_verified` для legacy-compatible diagnostics поверх existing transport-facing `customer.metadata.vk_peer_id`;
- VK ID OAuth state переключён на VK-safe compact формат `{base64url-payload}{43-char-signature}` без разделителя ([`buildSignedState`](../medusa-agency-boilerplate/src/modules/vk-id.ts:489)); [`verifySignedState`](../medusa-agency-boilerplate/src/modules/vk-id.ts:501) сохраняет backward-compatible read для legacy `payload.signature` формата (только для local/direct smokes, VK сам удаляет точку перед callback);
- новый Store API endpoint `POST /store/customers/me/onboarding` ([`route.ts`](../medusa-agency-boilerplate/src/api/store/customers/me/onboarding/route.ts)) принимает `{ email?, phone? }` в strict zod-схеме; phone нормализуется (удаляются пробелы/дефисы/скобки) и валидируется как `+7XXXXXXXXXX`/`8XXXXXXXXXX`/`+XXXXXXXXXX..XXXXXXXXXXXXXXX`. Email обязателен только когда current email — placeholder. Контракт ошибок: `email_required` (400), `email_already_set` (400), `email_already_exists` (409), `onboarding_already_complete` (400), `customer_auth_required` (401), `customer_not_found` (404), `update_failed`/`internal_error` (500). GET намеренно не реализован — storefront читает onboarding metadata из `GET /store/customers/me`;
- checkout-gate middleware [`enforceOnboardingEmailForCheckout`](../medusa-agency-boilerplate/src/modules/onboarding-checkout-gate.ts:19) применён только к `POST /store/carts/:id/complete` ([`middlewares.ts`](../medusa-agency-boilerplate/src/api/middlewares.ts:392)); blocks completion для аутентифицированных customer'ов с placeholder email, отдавая `{type:"invalid_data", code:"onboarding_required", details:{reason:"placeholder_email", action:"complete_onboarding"}}`. Guest checkout игнорируется (cart использует свой email); fail-open при инфраструктурной ошибке lookup-а — middleware сознательно не блокирует заказы при недоступности БД;
- storefront дополнения: страница [`/account/onboarding`](../medusa-agency-boilerplate-storefront/src/app/[countryCode]/(main)/account/onboarding/page.tsx), компоненты [`OnboardingBanner`](../medusa-agency-boilerplate-storefront/src/modules/account/components/onboarding-banner/index.tsx) и [`OnboardingForm`](../medusa-agency-boilerplate-storefront/src/modules/account/components/onboarding-form/index.tsx), helper [`isOnboardingPending`](../medusa-agency-boilerplate-storefront/src/lib/util/onboarding.ts), server action `submitOnboarding` в [`customer.ts`](../medusa-agency-boilerplate-storefront/src/lib/data/customer.ts). Banner появляется на `/account/profile`, не блокирует навигацию; форма требует email (когда placeholder) и оставляет phone опциональным с подсказкой о VK;
- storefront fix: anonymous посетитель `/account/profile` теперь редиректится на `/{countryCode}/account` вместо `notFound()` (commit `15d6304`);
- baseline-safe behaviour preserved: при `VK_ID_ENABLED=false` ни registration branch, ни onboarding endpoint, ни checkout gate не нужны для clean local onboarding и не блокируют storefront/backend startup. Onboarding endpoint существует независимо от VK ID и просто ничего не сделает для customer'ов без `metadata.onboarding`.
...
- `unisender` реализован как opt-in requested provider value для production-like email path в `v1`, а runtime по-прежнему отдельно фиксирует requested и resolved provider semantics;
...
- Storefront normal checkout must call direct ApiShip Store API surfaces and commit the standard Medusa shipping method with ApiShip data.
- Removed-provider env flags are not current required env.
- Старые obsolete provider ids/rows can exist only in previously used local/staging DBs and require separate operator-approved cleanup; destructive DB migration in this package is absent.
- Ни один markdown-документ этого репозитория не должен содержать фактическое значение ApiShip credentials, provider credentials, provider tokens, ciphertext, or raw provider payloads.
...
- `NEXT_PUBLIC_MEDUSA_BACKEND_URL` и `NEXT_PUBLIC_MEDUSA_BACKEND_PORT` используются как public/browser-safe inputs; server-side storefront runtime при этом сначала читает `MEDUSA_BACKEND_URL`, затем `NEXT_PUBLIC_MEDUSA_BACKEND_URL`, затем локальный порт fallback в [`src/lib/env.ts`](../medusa-agency-boilerplate-storefront/src/lib/env.ts:7).
...
- в staging production-mode container `MEDUSA_BACKEND_URL`/`DOCKER_MEDUSA_BACKEND_URL` должен указывать на Docker-network backend `http://medusa-backend:9000`, а `NEXT_PUBLIC_MEDUSA_BACKEND_URL` может указывать на public Caddy origin `https://studio.slavx.ru`;
...
- синхронизирует локальные ignored env-файлы `medusa-agency-boilerplate/.env` и `medusa-agency-boilerplate-storefront/.env.local` из root `.env`, включая YooKassa guardrails `YOOKASSA_STOREFRONT_RETURN_ORIGINS`, `YOOKASSA_WEBHOOK_URL`, `YOOKASSA_WEBHOOK_SECRET` и `YOOKASSA_ALLOW_UNSIGNED_WEBHOOKS`;
- поднимает PostgreSQL и Redis;
- запускает Medusa migration flow для пустой или уже существующей БД;
- запускает application-level seed из [`seed.ts`](../medusa-agency-boilerplate/src/scripts/seed.ts:166);
...
Guardrail:
- если локальный ignored `medusa-agency-boilerplate-storefront/.env.local` все еще содержит `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY=REPLACE_WITH_ROOT_BOOTSTRAP`, то [`preflight.sh`](../scripts/preflight.sh) специально валит preflight и требует сначала выполнить `npm run bootstrap`;
- если bootstrap падает на конфликтном seed-state, [`bootstrap.sh`](../scripts/bootstrap.sh) теперь завершает команду с ошибкой и не обновляет `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY` в storefront env по неполному или невалидному seed-output;
...
Примечание:
- шаблонный [`.env.example`](../.env.example) и текущий ignored root `.env` сейчас выровнены на backend `9000` и storefront `8000`;
...
Статус по ApiShip/Gorgo и checkout для этого канонического пути:
...
- obsolete provider env activation surface отсутствует: backend startup и clean onboarding не должны зависеть от obsolete provider credentials, mode flags или восстановленных activation semantics;
- старые базы могут содержать historical obsolete provider rows/provider ids; это считается только historical DB residue и очищается отдельно, external/operator-approved процедурой при необходимости;
