...
### 2. Первый post-order notification slice реализован, но ещё требует отдельной validation-фиксации

Что это значит технически:
для `order.placed` path контракт уже определен: canonical recipient = `order.email`, path = `subscriber → workflow → Notification Module`, controlled skip используется при отсутствии email, fallback chain в `v1` не применяется, а hardening v1.1 добавляет anti-duplicate contract поверх existing notification storage с canonical identity по `trigger_type + resource_type + resource_id + channel + template + normalized recipient`. Duplicate suppression трактуется как controlled skip с diagnostics, а race window в query-before-create path остается accepted limitation. Следующий шаг теперь не в реализации самого slice, а в его validation и фиксации failure signals без смешения с authenticated smoke baseline.

Простыми словами:
маршрут покупки уже доведен до первой реакции системы после оформления заказа, но это ещё нужно отдельно провалидировать и закрепить как verification result.
...
6. order lifecycle notifications v1 — уже реализован как первое customer-facing уведомление `order.placed` на подтверждённом order placement path, а hardening v1.1 синхронизирован как действующий anti-duplicate contract;
7. следующий внутренний шаг — **validation order lifecycle notifications v1** и затем commit, а не повторное проектирование notification/payment/shipping выбора;
8. Фаза 4 читается как следующий крупный слой интеграционного контура уже после validation этого первого post-order slice, а не как повод заново спорить о payment/shipping выборе;
9. Фазы 5-5.5 — пока не начинать как основные, пока не подтверждены ближайшие post-order tracks Фазы 4.
...
- order lifecycle notifications v1 уже реализован на path `subscriber → workflow → Notification Module` с trigger `order.placed`, recipient `order.email` и controlled skip при отсутствии email, а hardening v1.1 добавляет dedupe на existing notification storage без отдельного ledger;
- следующий внутренний шаг по умолчанию — это **validation order lifecycle notifications v1** от подтвержденного `order.placed` path и затем отдельный commit;
