...
**Вердикт:** ключевые integration slices подтверждены для текущего scope; notifications закрыты как первый slice и доведены до notification hardening v1, payments подтверждены для текущего YooKassa-first scope, shipping подтвержден как работающий ApiShip-first slice `cheapest_only_v1`, а checkout end-to-end validation v1 теперь тоже закрыт. Следующий шаг по sequencing — не новый выбор payment/shipping направления, а первый post-order customer-facing slice на уже подтвержденном order placement path.
...
- удерживать как source of truth, что notification hardening v1 уже закрыт, bootstrap idempotency hardening v1 **подтвержден runtime validation**, ApiShip `cheapest_only_v1` подтверждён runtime-проверкой, checkout end-to-end validation v1 закрыт, а regression-pack уже формализован;
- провести **order lifecycle notifications v1**: привязать первое customer-facing событие `order.placed` к уже подтверждённому order placement path;
- зафиксировать минимальный runtime contract и failure signals для этого post-order notification path без большого CI-redesign;
...
- customer-facing уведомление `order.placed` на реальном order placement path;
...
### 1. Checkout path подтверждён, но post-order notification lifecycle ещё не собран

Что это значит технически:
по notifications путь v1 уже подтвержден и доведен до hardening v1, по payments YooKassa-first path подтвержден для текущего scope, по shipping выбранный и реализованный ApiShip-first rate-selection slice подтверждён runtime-проверкой, а checkout chain уже подтверждена до confirmed order page. Но customer-facing событие `order.placed` ещё не зафиксировано как канонический runtime path поверх этого order creation.

Простыми словами:
клиент уже может дойти до подтвержденного заказа, но репозиторий ещё не доводит этот успех до первого обязательного post-order уведомления.

### 2. Первый post-order notification slice ещё не дошел до подтвержденного runtime contract

Что это значит технически:
теперь есть подтвержденный order placement path, значит логичный следующий слой — lifecycle notification от события `order.placed`. Для этого не нужно заново выбирать notification provider, payment provider или shipping provider, но нужно согласовать trigger, payload, fallback semantics и operational smoke для первого customer-facing post-order уведомления.

Простыми словами:
маршрут покупки уже доведен до конца, но реакция системы после оформления заказа пока не оформлена как подтвержденный шаблонный путь.
...
### 5. Нет productized storefront и release-контура

Что это значит технически:
storefront все еще требует отдельной productization-фазы, а шаблон не упакован в staging and prod-ready процесс тиражирования.

Простыми словами:
ядро стало честнее, но витрина и выпуск нового клиентского проекта пока не превращены в конвейер.
...
6. следующий внутренний шаг — **order lifecycle notifications v1**, то есть первое customer-facing уведомление `order.placed` на уже подтверждённом order placement path;
7. Фаза 4 читается как следующий крупный слой интеграционного контура уже после этого первого post-order slice, а не как повод заново спорить о payment/shipping выборе;
8. Фазы 5-5.5 — пока не начинать как основные, пока не подтверждены ближайшие post-order tracks Фазы 4.

Простыми словами:
план больше не нужно корректировать так, будто стабилизация и baseline еще не сделаны, будто Фаза 3 еще только начинается, будто notification hardening все еще открыт, будто payment track существует только как решение на бумаге, будто shipping остается на чистом decision stage, будто ApiShip blocker еще pending, будто checkout E2E ещё не закрыт, или будто bootstrap idempotency остается незакрытым hardening concern. Теперь нужно использовать уже подтвержденный order placement path как опору для первого post-order notification slice и только потом двигаться дальше по более широким storefront и release-направлениям.
...
- следующий внутренний шаг по умолчанию — это **order lifecycle notifications v1** от подтвержденного `order.placed` path;
...
> У нас уже есть подтвержденный clean onboarding, подтвержденный template-ready backend baseline без demo-oriented bootstrap, подтвержденный notification slice v1 как первый integration slice Фазы 3, закрытый notification hardening v1 с baseline-safe fallback и authenticated smoke, подтвержденный payment v1 как YooKassa-first path для текущего scope, реализованный первый backend-first ApiShip shipping slice как opt-in / baseline-safe rate-selection path и подтвержденный bootstrap idempotency hardening v1 (runtime validation `2026-04-17`). Следующий реальный шаг — формализовать один канонический regression-pack/source-of-truth для локальной проверки template readiness, не ждать внешнего ApiShip unblock как единственного направления работы; после внешней активации connections and contracts в ApiShip нужно повторить runtime validation цепочки `shipping → payment → review`, после чего двигаться к полному общему commerce-контуру, storefront core, отдельному Payload content layer, клиентской кастомизации и release-упаковке.
