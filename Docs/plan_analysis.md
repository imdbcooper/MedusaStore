...
> Delivery baseline status: this is an audit/history document. Current fresh-template delivery baseline is ApiShip/Gorgo via `@gorgo/medusa-fulfillment-apiship`; direct `/store/apiship/*` is canonical.

> Где смотреть текущий операционный статус:
> [`README.md`](../README.md), [current_work.md](./current_work.md), [architecture.md](./architecture.md), [production_runbook.md](./production_runbook.md), [local_development.md](./local_development.md), [staging_runbook.md](./staging_runbook.md), [troubleshooting.md](./troubleshooting.md)
>
> Staging/runtime sync `2026-05-16`: older statements below about missing production packaging/proxy/deploy automation are historical. Current implementation has [`docker-compose.prod.yml`](../docker-compose.prod.yml), Caddy-only reverse proxy, Payload staging container, manual GitHub Actions `Deploy Staging` workflow, and dynamic product runtime smoke requirements. Real production is not provisioned yet.
>
...
- **shipping track подтвержден для текущего scope**: текущий baseline — ApiShip/Gorgo через direct `/store/apiship/*`.
...
- verification pass подтвердил, что baseline and no-secret startup path не сломан, notification runtime не показал подтвержденной регрессии, а env-contract остался opt-in и baseline-safe;
...
- current delivery baseline теперь документирован через ApiShip/Gorgo direct Store API;
...
Простыми словами:
фундамент и template-ready backend baseline уже подтверждены, notification path v1 подтвержден и дополнительно доведен до закрытого notification hardening v1, payment path v1 уже подтвержден для текущего YooKassa-first scope, а текущий fresh-template shipping baseline теперь ApiShip/Gorgo через direct `/store/apiship/*` и standard Medusa shipping-method commit with `apishipData`. Checkout path подтверждён для текущего scope, но полный browser checkout smoke именно против живого ApiShip runtime не заявляется; для ApiShip/Gorgo baseline источники правды — [`apiship_direct_migration_plan.md`](./apiship_direct_migration_plan.md) и [`apiship_baseline_smoke_evidence.md`](./apiship_baseline_smoke_evidence.md). Bootstrap idempotency hardening v1 подтвержден runtime validation и больше не является открытым concern. Sequencing после закрытых storefront slices тоже стал уже not theoretical: `Фаза 6` была доведена от sanctioned selector [`NEXT_PUBLIC_STOREFRONT_PRESET`](../medusa-agency-boilerplate-storefront/src/lib/env.ts:21) к typed landing, adjacent product, listing/card, global-shell и catalog-shell surfaces, а финальный cross-preset regression pass зафиксировал весь этот stack как **PASS** и готовый к следующему roadmap stage.
...
**Вердикт:** ключевые integration slices подтверждены для текущего scope; notifications закрыты как первый slice и доведены до notification hardening v1, payments подтверждены для текущего YooKassa-first scope, shipping baseline теперь ApiShip/Gorgo direct `/store/apiship/*` для fresh templates, checkout validation закрыт для текущего YooKassa/runtime scope, `UniSender email migration v1` реализован и подтвержден, `post-UniSender cleanup-step` закрыт, `VK Community Messaging v1 foundation` уже реализован, `storefront core baseline v1` уже закрыт, `VK ID v1` уже реализован, повторно reviewed и закоммичен, а `MTS Exolve` уже реализован, повторно провалидирован и закоммичен. ApiShip/Gorgo live browser checkout и live shipment execution не заявляются как закрытые без отдельного scoped smoke/opt-in.
...
- shipping track current baseline: ApiShip/Gorgo direct Store API through `/store/apiship/*` and standard Medusa shipping-method commit with ApiShip data;
  ...
  - historical provider-specific runtime notes below are retained only as audit context for old database/runtime residue, not as current fresh-template behavior;
  - current fresh-template semantics are ApiShip/Gorgo pickup-point-first, direct `/store/apiship/*`, and default-off live shipment execution;
  - current ApiShip/Gorgo evidence is [`apiship_baseline_smoke_evidence.md`](./apiship_baseline_smoke_evidence.md); full live browser checkout and live shipment execution remain separately scoped.
- env-contract для integration path расширен opt-in переменными:
  ...
  - `UNISENDER_API_KEY`;
  - `UNISENDER_BASE_URL`.
- verification pass после первого historical delivery provider slice подтвердил:
  - `npm run bootstrap`, `npm run preflight` и `npm run dev` продолжают работать без обязательных payment secrets, без обязательных credentials удалённого historical delivery provider и без обязательных notification secrets;
...
- YooKassa уже является подтвержденным payment v1 direction, а fresh-template shipping baseline теперь ApiShip/Gorgo direct `/store/apiship/*`, если пользователь явно не сменил рынок/требования.
...
### 4. Нельзя держать исторический delivery blocker в статусе pending

Проблема:
исторический provider-specific blocker больше не является внешним account-state ожиданием для текущего fresh-template baseline. Текущий delivery baseline — ApiShip/Gorgo direct `/store/apiship/*`; old provider rows/ids в старых базах считаются operator-approved cleanup scope, а не active template behavior. Если продолжать описывать historical provider как pending blocker, документация снова начнет искажать реальный sequencing.

Простыми словами:
к старому delivery provider не нужно возвращаться как к незакрытому blocker'у; текущий source-of-truth delivery path — ApiShip/Gorgo.
...
Что это значит технически:
по notifications путь v1 уже подтвержден и доведен до hardening v1, по payments YooKassa-first path подтвержден для текущего scope, а по shipping текущий baseline зафиксирован как ApiShip/Gorgo direct Store API with default-off live shipment execution. Checkout chain подтверждена до confirmed order page для текущего runtime scope; полный ApiShip live browser checkout не заявляется без отдельного smoke. Поверх этого order creation уже реализован первый production-like customer-facing path: subscriber [`orderPlacedNotificationHandler()`](../medusa-agency-boilerplate/src/subscribers/order-placed-notification.ts:5) слушает `order.placed`, workflow [`sendOrderPlacedNotificationWorkflow`](../medusa-agency-boilerplate/src/workflows/send-order-placed-notification.ts:147) делает query по минимальной форме `{ id, display_id, email }`, а Notification Module отправляет notification с template `order-placed-v1` и trigger type `order.placed.customer.notification_requested`.
...
3. Фаза 3 — integration slices подтверждены: notification hardening v1 закрыт, payment v1 подтвержден как YooKassa-first path, shipping fresh-template baseline теперь ApiShip/Gorgo direct `/store/apiship/*`; ApiShip live browser checkout/live shipment execution не заявляются закрытыми без отдельного scoped smoke/opt-in;
