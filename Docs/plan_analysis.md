...
- зафиксировать минимальный runtime contract и failure signals для path `order.placed` без большого CI-redesign;
...
Что это значит технически:
для `order.placed` path контракт уже определен: canonical recipient = `order.email`, path = `subscriber → workflow → Notification Module`, controlled skip используется при отсутствии email, а fallback chain в `v1` не применяется. Следующий шаг теперь не в реализации самого slice, а в его validation и фиксации failure signals без смешения с authenticated smoke baseline.
...
6. order lifecycle notifications v1 — уже реализован как первое customer-facing уведомление `order.placed` на подтверждённом order placement path;
7. следующий внутренний шаг — **validation order lifecycle notifications v1**, а не повторное проектирование notification/payment/shipping выбора;
...
- order lifecycle notifications v1 уже реализован на path `subscriber → workflow → Notification Module` с trigger `order.placed`, recipient `order.email` и controlled skip при отсутствии email;
- следующий внутренний шаг по умолчанию — это **validation order lifecycle notifications v1** от подтвержденного `order.placed` path;
