...
- **shipping track v1** реализован как backend-first **ApiShip** provider-aware multi-quote slice; **`provider_aware_v1` подтверждён runtime-проверкой `2026-04-20`**: storefront получает grouped quotes из [`/store/apiship/rates`](../medusa-agency-boilerplate/src/api/store/apiship/rates/route.ts), группирует их по службе, сохраняет выбранный тариф в cart и backend уважает выбранный тариф; live multi-provider switching на тестовом адресе пока не подтверждено, потому что ответ вернул один provider group, но provider/tariff selection contract уже materialized;
...
- shipping slice v1 реализован как backend-first ApiShip rate-selection slice:
  ...
  - текущая checkout-семантика честно зафиксирована как `provider_aware_v1`: storefront и backend работают с grouped quotes, выбранным provider/tariff и отказом от silent cheapest-only fallback.
