...
- частично done: страница `Settings -> Delivery` больше не является planned-only и сейчас даёт `Providers`, `Connections`, `Yandex connection`, `Warehouses`, `Connection diagnostics`, `Test Quote`, встроенный read-only блок `Event logs`; поля `Enabled`, `Auto confirm`, `Label format` сохраняются в connection config, а `Default warehouse` теперь выбирается через materialized warehouse entity и резолвится в provider warehouse id внутри backend service;
- реально существующие базовые tests: [src/workflows/__tests__/delivery-hub.unit.spec.ts](../medusa-agency-boilerplate/src/workflows/__tests__/delivery-hub.unit.spec.ts) покрывает encryption / registry / provider listing, warehouse-aware connection serialization, warehouse mapping в `testQuote` и validation guardrails, а [src/workflows/__tests__/delivery-hub-admin.unit.spec.ts](../medusa-agency-boilerplate/src/workflows/__tests__/delivery-hub-admin.unit.spec.ts) покрывает admin list/create/update flows для connections/warehouses, successful `test-quote` response shape и stable error serialization with correlation details;
- частично done на shopper/store side: backend уже materializes neutral public contract routes [`GET /store/delivery/catalog`](../medusa-agency-boilerplate/src/api/store/delivery/catalog/route.ts), [`GET /store/delivery/quotes`](../medusa-agency-boilerplate/src/api/store/delivery/quotes/route.ts), [`GET /store/delivery/pickup-points`](../medusa-agency-boilerplate/src/api/store/delivery/pickup-points/route.ts), [`GET /store/delivery/pickup-windows`](../medusa-agency-boilerplate/src/api/store/delivery/pickup-windows/route.ts), [`GET /store/delivery/readiness`](../medusa-agency-boilerplate/src/api/store/delivery/readiness/route.ts) и selection surface [`GET/POST/DELETE /store/delivery/selection`](../medusa-agency-boilerplate/src/api/store/delivery/selection/route.ts); новый catalog contract возвращает shopper-usable connections через safe neutral поля `connection_id`, `label`, `state`, `ready`, `quote_types`, `supports_pickup_points`, `supports_pickup_windows`, `supports_dropoff` и optional `default_connection_id`, при этом intentionally не раскрывает `provider_code`, `credentials_state`, raw `enabled/status` fragments, `quote_key`, `raw_reference` и другие provider/internal детали; readiness contract возвращает только neutral `status + issues[] + selection + quote_context`, причём `quote_context.connection` intentionally ограничен safe summary `connection_id + state + ready`, проверяет persisted selection shape и shopper-facing connection readiness, но ещё не делает live re-quote или shipping-method lifecycle validation; текущий scope intentionally узкий — public surface работает только поверх materialized `Yandex Delivery` connection и minimal warehouse mapping, catalog default hint и auto-select используют only-one-ready-connection semantics, а checkout/storefront integration ещё не materialized;
- deferred: storefront cut-in, fulfillment-provider wiring, shipment/order widgets, live Yandex integration tests, ApiShip cutover/removal и mature multi-provider orchestration; при этом warehouse surface пока intentionally minimal — без sync из провайдера, без mode matrix, без shipment lifecycle — а logs surface остается узким read-only scope без pagination/detail/export/replay/delete;
- известный contract nuance зафиксирован truthfully: успешный `Test connection` отдаёт correlation context внутри diagnostics, а успешный `Test quote` — отдельным top-level `correlation_id`; оба значения пишутся в event log storage и доступны через `GET /admin/delivery/logs`, но более широкий ops contour для logging остаётся deferred.
...
- **`delivery-hub` tranche-1 materialized как параллельный foundation layer**: direct `Yandex Delivery` admin path, encrypted connection handling, admin page `Settings -> Delivery`, minimal local warehouse domain и базовые unit tests уже есть; storefront cutover, shipment domain, provider-driven warehouse sync/import и полная замена ApiShip остаются deferred и не должны overclaim'иться.
