...
- следующий logical roadmap slice внутри `Фазы 7` теперь лежит уже после tranche 1: template release checklist, onboarding doc и cleaned template release/package path из [`Docs/master_repo_plan_v2.md`](./master_repo_plan_v2.md); этот slice ещё не materialized и не должен смешиваться с уже закрытым tranche 1.
