...
- **Этап E (Shipment operations)** — current state is still preview-only/no-network, but now includes two materialized backend-only slices. First, pure helper [`materializeYandexCreateShipmentPayloadPreview()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-materializer.ts:196) детерминированно собирает redacted/safe future Yandex `create_shipment` payload preview из уже persisted backend-only handoff/provider-origin execution context для [`warehouse_to_pickup_point`](../medusa-agency-boilerplate/src/modules/delivery-hub/constants.ts:30) и [`dropoff_point_to_pickup_point`](../medusa-agency-boilerplate/src/modules/delivery-hub/constants.ts:31). Он возвращает `ready|blocked` и explicit blockers `missing_or_unsupported_mode`, `missing_provider_origin_dispatch_context`, `wrong_origin_context_for_mode`, `missing_destination_pickup_point`, `missing_pickup_interval_window`, `missing_recipient_contact`, `missing_package_items`; preview не содержит credentials/auth headers/raw backend execution token/raw provider payload and masks provider/internal references. Second, [`buildDeliveryHubControlledFulfillmentExecutionResult()`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts) now wires that helper into the final controlled `createFulfillment` seam as preview-only evidence and emits redacted `provider_payload_materialization` summary (`status`, `attempted`, `ready`, `blocked_reason_code`, `blocked_reason`, provider execution reference/context presence flags, redacted preview-summary counts/booleans and explicit anti-leak confirmations). The helper is invoked only after the seam has reached a meaningful provider-dispatch-contract stage; earlier blocked stages stay `not_attempted`. Even when payload preview is `ready`, runtime remains intentionally inert: no live Yandex adapter invocation, no provider network dispatch, no shipment persistence, no execution-ledger persistence activation, no order/fulfillment mutation and `live_adapter_call_performed=false`.
...
- §16.17 `Этап E` синхронизирован с materialized backend-only wiring of [`materializeYandexCreateShipmentPayloadPreview()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-materializer.ts:196) into [`buildDeliveryHubControlledFulfillmentExecutionResult()`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts), including preview-only `provider_payload_materialization` evidence and explicit `not_attempted|blocked|ready` semantics;
- tests/status narrative переписан так, чтобы explicitly удерживать factual boundaries текущего shipment contour: payload materialization readiness can now be previewed truthfully for direct Yandex execution, but live adapter invocation/provider network dispatch/shipment persistence/execution-ledger activation/order mutation remain disabled.
