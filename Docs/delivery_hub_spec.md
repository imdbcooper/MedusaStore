...
Current factual baseline: direct Yandex dispatch exists, accepted shipment persistence exists, execution-ledger activation exists, replay/duplicate/drift blockers are hardened, failed path remains failed-blocked, shipment persistence exists only for accepted dispatch outcome, accepted shipment lifecycle read-model exists, and backend-only Yandex status polling v1 can refresh only accepted persisted shipments with redacted neutral status markers. This roadmap does not claim implemented webhooks, cancel, retry automation, broad admin shipment operations, checkout rewrite or additional providers.
...
- this change does not implement webhooks, cancel, retry scheduling/queue, background worker fleet, broad admin/store shipment operations, checkout rewrite, new provider support or new required env; next implementation items remain the minimal admin shipment operations panel, cancel path policy, manual retry policy and webhook/post-v1 lifecycle work.
