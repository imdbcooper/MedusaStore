...
>
> Previous-baseline Phase 8 status (2026-05-01): at the time of this historical spec, shopper checkout was described as a Delivery Hub delivery flow: quote/PVZ selection, save delivery method, matched Medusa shipping option handoff, then payment only when delivery was ready. That statement is retained as historical phase evidence only, not current checkout guidance. Older preview/shadow/cutover sections in this specification are historical or advanced dev/admin diagnostics from the previous Delivery Hub baseline. They must not be used to reintroduce shopper-visible diagnostic vocabulary, a legacy delivery fallback path, or Delivery Hub checkout behavior into the current ApiShip/Gorgo baseline.
...
Успешный `Test quote` response в текущем контракте возвращает `correlation_id` top-level и structured `diagnostics_summary`; для `Test connection` correlation context приходит внутри `diagnostics.correlation_id` и дополнительно summary-level. Этот contour означает `live-validation readiness/admin diagnostics hardened`, но не включает production shipment execution.
...
Historical previous-baseline intermediate status at the time of this spec:
 
- shopper checkout in the old Delivery Hub phase still used legacy provider shipping-method persistence in storefront [`setShippingMethod()`](../medusa-agency-boilerplate-storefront/src/lib/data/cart.ts:226) and checkout UI [`Shipping`](../medusa-agency-boilerplate-storefront/src/modules/checkout/components/shipping/index.tsx:299);
- delivery-hub selection existed as a separate neutral cart metadata contract through [`GET/POST/DELETE /store/delivery/selection`](../medusa-agency-boilerplate/src/api/store/delivery/selection/route.ts), but shopper-facing cutover was not performed in that historical tranche;
- storefront materialized only read-only readiness visibility, read-only summary visibility, read-only persisted-selection visibility, sampled read-only shadow catalog visibility, sampled read-only shadow pickup-point visibility, sampled read-only shadow quote visibility, and dependent read-only shadow pickup-window visibility through helper-level preview models without intervening in the then-active shipping selection flow.
...
- `correlation_id` обязан:
  ...
  - возвращаться в merchant-facing response contract:
    - `testConnection` → `diagnostics.correlation_id` plus redacted `diagnostics_summary`;
    - `testQuote` → top-level `correlation_id` plus redacted `diagnostics_summary` и safe `input_echo`.
...
- Historical note: Delivery Hub/direct Yandex was the selected/default delivery contour at the time of this previous-baseline spec; current template baseline is ApiShip/Gorgo. `manual` and `deliveryhub` references in this section describe the previous-baseline state at the time of the spec; current runtime guidance is ApiShip/Gorgo and Delivery Hub quarantine/cleanup docs.
- Historical previous-baseline note: shopper-facing `shipping_option.provider_id` with value `deliveryhub` was treated as the intended direct contour for the then-planned storefront template, while live shipment dispatch remained gated/not enabled. This is not current storefront-template guidance after the ApiShip/Gorgo migration; the current standard shipping method commit remains generic/provider-neutral.

### 16.14. Historical storefront / checkout migration mapping

#### 16.14.1. Historical checkout boundary

> **Previous-baseline state on `2026-04-23`:** storefront no longer used old provider-specific routes/helpers in the Delivery Hub phase. Delivery Hub neutral selection save/clear/summary was present through [`saveDeliveryHubSelection()`](../medusa-agency-boilerplate-storefront/src/lib/data/delivery-hub.ts:151), [`clearDeliveryHubSelection()`](../medusa-agency-boilerplate-storefront/src/lib/data/delivery-hub.ts:169), and summary surfaces; standard shipping method commit through [`setShippingMethod()`](../medusa-agency-boilerplate-storefront/src/lib/data/cart.ts:226) remained generic/provider-neutral and had to use existing Medusa shipping option ids. Live shipment dispatch remained gated/not enabled, and removed provider/routes/helpers were gone from that template/runtime snapshot. This paragraph is retained as historical evidence only and is not current checkout guidance for the ApiShip/Gorgo baseline.
...
| 20 | [`buildDeliveryHubNeutralSelectionRehearsalModel()`](../medusa-agency-boilerplate-storefront/src/lib/util/delivery-hub.ts) + [`evaluateDeliveryHubNeutralSelectionRehearsalActionability()`](../medusa-agency-boilerplate-storefront/src/lib/util/delivery-hub.ts) | current | Pre-cutin neutral selection rehearsal: `candidate_available / blocked / insufficient_data / legacy_only`, dry-run body-shaping verdict only |

#### 16.14.3. Read-only boundary guarantees

Все 19 shadow preview builders плюс pre-cutin neutral selection rehearsal layer:
...
Targeted storefront coverage в [`delivery-hub.spec.ts`](../medusa-agency-boilerplate-storefront/src/lib/util/delivery-hub.spec.ts) фиксирует status paths для всех materialized preview builders, включая `ready / pending / blocked / insufficient_data` paths для checklist preview, а также neutral rehearsal paths `candidate_available / blocked / insufficient_data / legacy_only`, dry-run/no-mutation intent и no-leak guarantees.
...
- **Этап A (Foundation)** — materialized. Реализованы: module scaffold, `delivery_connection`/`delivery_event_log` storage, encryption, registry, adapter interface, historical Delivery Hub/direct Yandex delivery contour.
- **Этап B (Admin foundation)** — частично. Реализовано: materialized `Settings -> Delivery`, `Providers` list, `Connections` CRUD, `Yandex connection` form, minimal local `Warehouses` CRUD, `Test connection`, `Connection diagnostics` с hardened structured diagnostics, admin-only read preview [`GET /admin/delivery/shipping-options/preview`](../medusa-agency-boilerplate/src/api/admin/delivery/shipping-options/preview/route.ts), admin-only manual sync scaffold [`POST /admin/delivery/shipping-options/sync`](../medusa-agency-boilerplate/src/api/admin/delivery/shipping-options/sync/route.ts), summary-only audit trail для manual shipping-option sync через existing logs contour, встроенный `Shipping option preview` block на странице, operator-facing `Shipping option manual sync` UI block с safe-by-default `dry_run` и guarded execute path, а также узкий `Shipment operations lookup` UI/page-state slice для single `execution_reference` snapshot lookup, guarded `Refresh status`, backend-guarded manual `Cancel shipment` и backend-guarded manual `Retry execution` (readiness-first, blocked-only in v1). Не реализовано: `/connections/:id/disable` как отдельный роут, отдельный `Delivery Modes` surface, provider-driven warehouse sync/import, полноценный logs console beyond текущего read-only блока, live retry redispatch/webhook/scheduler shipment actions и broad shipment dashboard/table.
...
- **Этап E (Shipment operations)** — current state includes gated direct Yandex `create_shipment`, execution-ledger activation, persisted accepted shipment linkage, accepted shipment lifecycle read-model, backend-only Yandex status polling foundation for accepted persisted shipments, the first backend/admin shipment-operations visibility slice by execution reference, a first narrow React Admin page-state/UI integration for that same single-reference operator surface, and a backend/admin-only manual cancel policy/path v1 for accepted Yandex shipments. The first real direct Yandex `create_shipment` runtime dispatch still exists only behind a strict gate: runtime [`buildDeliveryHubControlledFulfillmentExecutionResult()`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts:186) and [`createFulfillment()`](../medusa-agency-boilerplate/src/modules/deliveryhub.ts:150) wire the existing Postgres repository [`DeliveryHubExecutionLedgerPgRepository`](../medusa-agency-boilerplate/src/modules/delivery-hub/storage/execution-ledger-pg-repository.ts:49) only around the narrow path `ready -> actual gated dispatch attempt -> accepted shipment result`. Pure helper [`materializeYandexCreateShipmentPayloadPreview()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-materializer.ts:196) still deterministically assembles a redacted/safe payload preview from persisted backend-only handoff/provider-origin execution context for [`warehouse_to_pickup_point`](../medusa-agency-boilerplate/src/modules/delivery-hub/constants.ts:30) and [`dropoff_point_to_pickup_point`](../medusa-agency-boilerplate/src/modules/delivery-hub/constants.ts:31). Narrow direct-Yandex dispatch primitives [`buildYandexCreateShipmentDispatchRequest()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:223) and [`executeYandexCreateShipmentDispatch()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:254) remain wired into that controlled runtime path. Accepted dispatches progress through the canonical ledger path and persist `delivery_shipments`; failed dispatches remain bounded at `failed_blocked` without creating a misleading shipment record. Replaying an already completed accepted execution identity, replaying a failed-blocked identity, duplicate exact non-terminal reservations and drifted reservations remain pre-dispatch zero-call blockers. Backend lifecycle helper [`shipment-lifecycle-read-model.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-lifecycle-read-model.ts) exposes sanitized accepted shipment snapshots only for persisted accepted rows. On that accepted-only foundation, [`shipment-status.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/shipment-status.ts) now provides an adapter-local conservative Yandex status request/executor/normalizer boundary, and [`refreshDeliveryHubAcceptedShipmentStatus()`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-status-polling.ts:102) gates status refresh by accepted lifecycle snapshot, accepted persisted shipment row, active/enabled/sealed Yandex connection and shipment storage connection before any provider call. Status polling writes only redacted markers through [`recordDeliveryShipmentStatusUpdate()`](../medusa-agency-boilerplate/src/modules/delivery-hub/storage/shipments-repository.ts): `provider_status_summary`, `status_refresh_outcome`, `status_refreshed_at` and a redacted metadata marker. Cancel readiness is resolved by [`resolveDeliveryHubShipmentCancelReadiness()`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-cancel-policy.ts:169): cancellation is available only for accepted lifecycle snapshots with a persisted accepted `delivery_shipments` row, active/enabled/sealed Yandex connection, supported provider `yandex`, a backend-only raw provider shipment reference and no unsafe neutral status (`cancelled`, `delivered`, `returned`, `failed`). [`cancelDeliveryHubAcceptedShipment()`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-cancel-policy.ts:106) blocks before provider call for non-accepted, missing-reference, missing/not-ready connection, unsupported provider, already-cancelled and terminal states; when allowed it calls the adapter-local conservative Yandex cancel seam [`executeYandexShipmentCancel()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/shipment-cancel.ts:91) and returns only a sanitized admin result. The local repository does not contain an authoritative direct-Yandex cancellation contract, so the adapter documents `semantics_certainty=adapter_boundary_mocked_only` and the conservative `/shipments/cancel` seam must be live-validated before production SLA/guarantee claims. The admin/backend operations surface [`admin-shipment-operations.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/admin-shipment-operations.ts) projects one `execution_reference` into a safe operator snapshot and routes [`GET /admin/delivery/shipments/:execution_reference/operations`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/route.ts) / [`POST /admin/delivery/shipments/:execution_reference/operations/refresh-status`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/refresh-status/route.ts) / [`POST /admin/delivery/shipments/:execution_reference/operations/cancel`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/cancel/route.ts) expose sanitized admin-only read/refresh/cancel boundaries. The React Admin extension now adds pure helpers/types in [`page-state.ts`](../medusa-agency-boilerplate/src/admin/routes/settings/delivery/page-state.ts) and a minimal `Shipment operations lookup` section in [`page.tsx`](../medusa-agency-boilerplate/src/admin/routes/settings/delivery/page.tsx): operator enters/pastes one `execution_reference`, loads the safe snapshot, sees lifecycle classification, accepted flag, provider/mode/status summary, refresh/cancel readiness, refresh outcome, ledger counts/previews, timestamps/context and blocked/action posture, and can trigger `Refresh status` or `Cancel shipment` only when backend view-model marks the action available. Provider statuses are normalized into neutral `accepted | in_transit | ready_for_pickup | delivered | cancelled | failed | returned | unknown`; unknown provider states are explicit and do not corrupt the accepted shipment snapshot. No raw provider response, auth headers, credentials, raw Yandex response body, execution token, quote key, raw provider identifier, raw provider shipment id, raw execution secret or secret-bearing payload is returned, persisted or rendered by the page-state display model. This is not a webhook path, not a background scheduler/queue, not a retry/compensation path, not a broad admin/store API/dashboard/table, and not a checkout rewrite.
...
Current factual baseline: direct Yandex dispatch exists, accepted shipment persistence exists, execution-ledger activation exists, replay/duplicate/drift blockers are hardened, failed path remains failed-blocked, shipment persistence exists only for accepted dispatch outcome, accepted shipment lifecycle read-model exists, backend-only Yandex status polling v1 can refresh only accepted persisted shipments with redacted neutral status markers, and the admin/backend shipment-operations slice now exposes sanitized execution-reference snapshot plus guarded refresh-status/cancel/retry actions. Cancel v1 is backend/admin-only, accepted-Yandex-only, redacted, blocked before provider calls for unsafe states, and its direct Yandex adapter semantics remain local/mock-covered until live validation. Manual retry v1 is backend/admin-only readiness-first and currently blocked-only for execution result: it allows operator request path only when strict retry readiness is safe, but does not materialize live provider redispatch in this tranche. This roadmap does not claim implemented webhooks, auto-retry/scheduler automation, broad React admin shipment dashboard/table, checkout rewrite or additional providers.
...
1. **Accepted shipment lifecycle read-model**
   - **Current status:** `materialized as backend-only narrow read/snapshot helper`; broader admin route/service surface remains planned for the later admin operations panel.
   ...
   - **Scope:** backend-safe read model over persisted accepted `delivery_shipments` plus execution-ledger linkage and existing controlled fulfillment execution context; snapshot exposes internal shipment identity where safe, provider/mode/status summary already persisted, accepted dispatch outcome markers, safe ledger linkage/reference previews, timestamps/update markers and fulfillment/order-ish context references already present in the execution contour. Non-accepted classifications are explicit for absent, provider-failed/failed dispatch, failed-blocked, completed replay, duplicate exact non-terminal and drifted contours.
   - **Explicit non-goals:** no polling, no provider status API call, no webhooks, no cancel, no retry, no provider re-dispatch, no label download unless already safely present, no failed-result shipment persistence, no storefront exposure and no broad public admin/store contract expansion in this tranche.
   - **Dependencies:** existing accepted shipment persistence, execution-ledger completed/failed-blocked/blocker states, redaction helpers and response-boundary sanitation.
   - **Definition of Done/evidence:** backend helper returns sanitized accepted shipment lifecycle snapshots only for persisted accepted shipment records; failed-blocked/replay/duplicate/drift/provider-failed contours remain non-shipment states and do not look accepted; focused no-network coverage proves shape, redaction and blocker preservation; docs/current status updated.

2. **Yandex status polling v1**
   - **Current status:** `materialized as backend-only accepted-shipment read/update foundation`; admin-trigger route/UI and scheduler remain planned later.
   ...
   - **Scope:** conservative direct Yandex status request boundary for already accepted persisted shipments, accepted-only gating through the lifecycle read-model and persisted row criteria, neutral status normalization, unknown-status safety, redacted status update markers in shipment storage, and safe backend result/log summaries. In the current narrow integration, accepted `createFulfillment()` results attach `accepted_shipment_status_refresh` after accepted lifecycle evaluation; non-accepted paths block before provider calls.
   - **Explicit non-goals:** no webhooks before polling baseline, no auto-retry of failed `create_shipment`, no broad scheduler fleet, no retry queue, no cancel path, no checkout changes, no broad admin/store API, no new provider and no required new env.
   - **Dependencies:** accepted shipment lifecycle read-model, accepted shipment persistence, existing Yandex client conventions, redaction helpers, execution-ledger blocker semantics and shipment storage.
   - **Definition of Done/evidence:** focused mocked provider coverage proves accepted refresh calls provider once for the status refresh and stores/returns a safe normalized summary; failed/replay/duplicate/drift/blocked/absent or otherwise non-accepted paths perform zero status provider calls; unknown provider statuses normalize safely as `unknown` without corrupting the accepted shipment snapshot; raw provider responses, auth/token fragments, quote keys and unsafe provider identifiers are not returned or persisted; docs state webhooks/cancel/retry/admin panel remain later.

3. **Minimal admin shipment operations panel**
   - **Current status:** `first backend/admin-only visibility slice materialized` plus `first narrow React Admin page-state/UI slice materialized`; broad panel/dashboard UX remains planned later.
   ...
   - **Scope now:** a narrow admin backend surface resolves one persisted shipment/execution reference into a sanitized operations snapshot via [`admin-shipment-operations.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/admin-shipment-operations.ts), [`DeliveryHubService.getAdminShipmentOperationsSnapshot()`](../medusa-agency-boilerplate/src/modules/delivery-hub/service.ts) and [`GET /admin/delivery/shipments/:execution_reference/operations`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/route.ts). It exposes accepted lifecycle classification, safe provider/mode/status summary, current neutral polling summary if already available, safe ledger linkage previews/counts, timestamps, explicit blocked reason for non-accepted/non-refreshable cases and action posture. The existing `Settings -> Delivery` extension now adds pure page-state URL/action/display helpers in [`page-state.ts`](../medusa-agency-boilerplate/src/admin/routes/settings/delivery/page-state.ts) and a minimal [`page.tsx`](../medusa-agency-boilerplate/src/admin/routes/settings/delivery/page.tsx) `Shipment operations lookup` section with one `execution_reference` input, `Load snapshot`, read-only safe display cards/rows and a guarded `Refresh status` button shown/enabled only from the backend/view-model action posture.
   - **Refresh-status slice:** [`POST /admin/delivery/shipments/:execution_reference/operations/refresh-status`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/refresh-status/route.ts) is included because it reuses the existing accepted-only guard in [`refreshDeliveryHubAcceptedShipmentStatus()`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-status-polling.ts:102). Accepted persisted shipment, active/enabled/sealed Yandex connection, storage connection and backend-only raw provider shipment reference are required before the provider call; missing reference, failed/replay/duplicate/drift/blocked/absent and other non-accepted paths stay zero-call.
   - **Explicit non-goals:** no official Admin fork, no broad 40-field form, no automatic sync loop/background polling, no raw provider payloads, no raw provider shipment id, no raw execution secret, no storefront/store exposure, no broad table/dashboard/filter/export, and no cancel/retry execution until their policies are separately approved.
   - **Dependencies:** read-model tranche, polling v1 output, existing admin auth, response-boundary sanitation, shipment/ledger repositories, Yandex connection readiness and existing admin extension route surface.
   - **Definition of Done/evidence:** focused backend tests in [`delivery-hub-admin-shipment-operations.unit.spec.ts`](../medusa-agency-boilerplate/src/workflows/__tests__/delivery-hub-admin-shipment-operations.unit.spec.ts) prove accepted snapshot shape, non-accepted blocking, refresh provider call only from backend-only provider reference, missing-reference zero-call behavior and anti-leak response/view-model boundaries; focused page-state coverage in [`page-state.unit.spec.ts`](../medusa-agency-boilerplate/src/admin/routes/settings/delivery/__tests__/page-state.unit.spec.ts) locks endpoint URL building, refresh body shaping, available/blocked display posture and anti-leak display-model behavior without network.

4. **Cancel policy/path v1**
   - **Current status:** `materialized as backend/admin-only manual accepted-Yandex cancel seam`; live Yandex endpoint semantics remain `adapter_boundary_mocked_only` until provider-contract validation.
   - **Why needed:** production operations need a controlled way to request cancellation for accepted shipments while preserving accepted-only lifecycle safety, zero-call blockers and anti-leak boundaries.
   - **Scope:** [`resolveDeliveryHubShipmentCancelReadiness()`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-cancel-policy.ts:169) exposes the cancel eligibility matrix in the safe operations snapshot. Cancellation is available only when the lifecycle snapshot is accepted, the persisted row is an accepted `delivery_shipments` record, provider/connection is supported Yandex and active/enabled/sealed, a backend-only raw provider shipment reference exists, and the latest neutral status is not `cancelled`, `delivered`, `returned` or `failed`. [`POST /admin/delivery/shipments/:execution_reference/operations/cancel`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/cancel/route.ts) is an explicit admin manual action, returns a safe cancel result plus refreshed operations snapshot, and never returns raw provider ids, request/response bodies, auth headers, credentials, quote keys, execution secrets or secret-bearing payloads. Already-cancelled posture is blocked from snapshot readiness when known before the provider call; if a provider response reports already-cancelled after an allowed call, the result is `already_cancelled` and not faked as a new provider success.
   - **Yandex certainty:** [`shipment-cancel.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/shipment-cancel.ts) defines a conservative direct-Yandex adapter seam and mocked no-network tests around `/shipments/cancel`, but the local docs/code did not contain an authoritative Yandex cancellation endpoint/contract. Therefore this tranche does not claim live-provider guarantee beyond the sanitized adapter boundary.
   - **Explicit non-goals:** no shopper self-service cancel, no order cancellation rewrite, no retry, no compensation automation, no background worker/scheduler/queue, no webhook, no broad dashboard/table, no storefront/store API, no new provider and no required new env.
   - **Dependencies:** polling v1, admin operations panel, accepted shipment read-model, existing Yandex connection readiness and response-boundary sanitation. Live provider rollout still depends on Yandex cancel endpoint validation and any future ledger/event transition extension.
   - **Definition of Done/evidence:** focused mocked/no-network coverage proves allowed accepted cancel, non-accepted/terminal/already-cancelled blocking, missing backend-only provider reference zero-call behavior, sanitized provider failure, endpoint URL encoding, UI disabled/available posture and anti-leak assertions for provider id/auth/token/quote/execution secret/raw body.

5. **Manual retry policy**
   - **Current status:** `materialized as backend/admin-only readiness-first retry v1 with blocked-only execution result`.
   ...
   - **Scope (current v1):** [`resolveDeliveryHubShipmentRetryReadiness()`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-retry-policy.ts:69) classifies retryable vs non-retryable posture and allows retry readiness only on a narrow safe contour: lifecycle not duplicate/drift/accepted, neutral status not in accepted/in_transit/ready_for_pickup/cancelled/delivered/returned/unknown, execution-ledger present in `failed_blocked` with transitions, idempotency reservation linkage intact, persisted diagnostic execution plan present, and duplicate guards preserved (no accepted shipment and no provider shipment reference marker). Admin service/action path [`retryAdminShipmentOperationsExecution()`](../medusa-agency-boilerplate/src/modules/delivery-hub/service.ts:469) + route [`POST /admin/delivery/shipments/:execution_reference/operations/retry`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/retry/route.ts) are admin-auth-protected and return only sanitized result/snapshot via [`shared.ts`](../medusa-agency-boilerplate/src/api/admin/delivery/shared.ts). Existing shipment-operations snapshot now exposes retry readiness and action posture; `Settings -> Delivery` shipment lookup adds a guarded Retry button enabled only when backend says retry is available.
   - **Execution semantics certainty (current v1):** [`requestDeliveryHubShipmentManualRetry()`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-retry-policy.ts:100) is intentionally blocked-only even when readiness passes (`blocked_reason_code=retry_execution_not_materialized`, `provider_call_attempted=false`, `semantics_certainty=readiness_only_no_live_redispatch`) to avoid weakening deterministic execution identity/idempotency and to avoid duplicate shipment risk until a proven live redispatch contract is approved.
   - **Explicit non-goals:** no auto-retry, no queue/scheduler, no provider redispatch from this v1 path, no weakening duplicate/drift/replay blockers, no fake provider success surface.
   - **Dependencies:** execution-ledger state model, polling/error taxonomy, accepted/failed read-model visibility, support runbook.
   - **Definition of Done/evidence:** focused no-network tests prove readiness allow/block matrix (including accepted/cancelled/delivered/returned/terminal blockers, missing ledger/idempotency blockers, drift/duplicate blockers), endpoint auth/URL wiring, UI availability posture and anti-leak boundaries; retry API remains truthful blocked-only until live redispatch is separately approved.
...
7. **Live Yandex validation matrix**
   - **Current status:** `manual-only harness/runbook materialized; Tranche B blocked preflight evidence captured; live evidence collection still pending prerequisites`.
   ...
   - **Scope:** operator-approved live/test-mode matrix for `warehouse_to_pickup_point` and `dropoff_point_to_pickup_point`, including quote, pickup points/windows, gated create-shipment acceptance, status polling, cancel where available, redaction checks and failure cases. Manual preparation contour is now materialized via harness [`delivery-hub-yandex-provider-contract-validation.ts`](../medusa-agency-boilerplate/src/scripts/delivery-hub-yandex-provider-contract-validation.ts), runbook [`delivery_hub_yandex_provider_contract_validation_runbook.md`](./delivery_hub_yandex_provider_contract_validation_runbook.md), and blocked preflight evidence bundle [`delivery_hub_yandex_provider_contract_validation_evidence_20260424.md`](./delivery_hub_yandex_provider_contract_validation_evidence_20260424.md).
   ...
   - **Definition of Done/evidence:** sanitized run log/evidence package with environment labels only, expected/actual matrix, PASS/BLOCKED outcomes, no secret fragments and linked docs update. Tranche B evidence currently captures fail-closed blockers (`live_opt_in_env_required`, `live_confirm_required`, `encryption_key_required`, `connection_not_found`) and does not claim live provider-call execution.

8. **Operational runbooks/support hardening**
   - **Current status:** `partial`.
   - **Why needed:** production readiness requires support procedures for gated dispatch, accepted shipment lifecycle, polling failures, cancel, manual retry readiness/block decisions and escalation without relying on implementer memory.
...
Любое изменение §§1–15 требует синхронного обновления §16. При расхождении §§1–15 vs §16 tooling/реализация должны следовать §16. §§1–15 служат долгосрочным roadmap-нарративом; §16 — truthful текущий контракт, синхронизированный с кодом на дату `2026-04-23`.

Изменения `2026-04-23`:

- §16.17 `Этап E` синхронизирован с materialized runtime wiring of [`materializeYandexCreateShipmentPayloadPreview()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-materializer.ts:191), dispatch-port contract [`buildYandexCreateShipmentDispatchPortContract()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:145) and executable primitives [`buildYandexCreateShipmentDispatchRequest()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:179) / [`executeYandexCreateShipmentDispatch()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:211) into [`buildDeliveryHubControlledFulfillmentExecutionResult()`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts:168) and runtime [`createFulfillment()`](../medusa-agency-boilerplate/src/modules/deliveryhub.ts:235);
- factual boundary is now stricter and narrower than before: live provider invocation on the fulfillment path exists only under the exact gate + supported-mode + provider-origin-ready + materialization-ready + connection-ready contour; completed replay, failed-blocked replay, duplicate exact non-terminal reservations and drifted reservations all block before provider dispatch with zero-call semantics; runtime logs/results stay redacted with no raw provider body, auth headers, execution token or quote key exposure;
- tests/status narrative переписан так, чтобы explicitly удерживать factual boundaries текущего shipment contour: direct Yandex runtime dispatch is now real but gated; narrow shipment persistence plus canonical execution-ledger reservation/result progression are materialized only on the already ready direct-Yandex contour; accepted dispatches may progress through `application_ready -> completed`, failed dispatches are truthfully bounded at `failed_blocked` without shipment persistence, completed replay and failed replay stay pre-dispatch zero-call blocked contours, duplicate/drift blockers remain pre-dispatch zero-call blocked contours, and retries, compensation, webhook/polling lifecycle and broad order/fulfillment mutation remain disabled or not implemented in this tranche.

Изменения `2026-04-24`:

- §16.17.1 step 1 updated from partial/planned read-model wording to a materialized backend-only read/snapshot tranche: [`shipment-lifecycle-read-model.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-lifecycle-read-model.ts) builds sanitized accepted shipment lifecycle snapshots from persisted accepted shipment records, execution-ledger linkage and existing controlled execution context, while [`getDeliveryShipmentByExecutionReference()`](../medusa-agency-boilerplate/src/modules/delivery-hub/storage/shipments-repository.ts) provides safe readback by execution reference and [`createFulfillment()`](../medusa-agency-boilerplate/src/modules/deliveryhub.ts:150) attaches the snapshot to the backend-only controlled result/log summary;
- the read-model deliberately classifies provider-failed, failed-blocked, completed replay, duplicate exact non-terminal and drifted contours as non-accepted so they cannot be mistaken for accepted shipment records, and it exposes only redacted/safe fields: persisted shipment id, provider/mode/status summary, accepted dispatch markers, safe ledger linkage/reference previews, timestamps and existing order/fulfillment/cart/shipping-option/location context references;
- §16.17 and §16.17.1 step 2 updated from planned polling wording to materialized backend-only Yandex status polling v1: [`shipment-status.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/shipment-status.ts) provides the conservative Yandex status adapter boundary and safe normalizer, [`refreshDeliveryHubAcceptedShipmentStatus()`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-status-polling.ts:102) gates provider calls to accepted persisted shipments, and [`recordDeliveryShipmentStatusUpdate()`](../medusa-agency-boilerplate/src/modules/delivery-hub/storage/shipments-repository.ts) persists only redacted status markers;
- §16.17 and §16.17.1 step 3 updated from planned-only admin shipment operations wording to a first backend/admin visibility slice: [`admin-shipment-operations.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/admin-shipment-operations.ts) builds the redacted operator snapshot, [`GET /admin/delivery/shipments/:execution_reference/operations`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/route.ts) returns the safe lifecycle/status/ledger view-model, and [`POST /admin/delivery/shipments/:execution_reference/operations/refresh-status`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/refresh-status/route.ts) reuses the accepted-only status polling guard without exposing raw provider identifiers or execution secrets;
- current tranche updates §16.17 and §16.17.1 step 3 from backend-only UI-planned wording to a first narrow React Admin page-state/widget integration: [`page-state.ts`](../medusa-agency-boilerplate/src/admin/routes/settings/delivery/page-state.ts) now has shipment operations form/types, endpoint builders, refresh request helper and safe display-state derivation, while [`page.tsx`](../medusa-agency-boilerplate/src/admin/routes/settings/delivery/page.tsx) adds a minimal `Shipment operations lookup` section for one `execution_reference`, load snapshot and backend-guarded `Refresh status` action;
- current tranche additionally materializes §16.17.1 step 4 as a backend/admin-only cancel policy/path v1: [`shipment-cancel-policy.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-cancel-policy.ts) resolves accepted-Yandex-only cancel readiness and executes guarded manual cancel, [`shipment-cancel.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/shipment-cancel.ts) defines the conservative direct-Yandex sanitized adapter seam, [`POST /admin/delivery/shipments/:execution_reference/operations/cancel`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/cancel/route.ts) exposes the explicit admin action, and the existing `Shipment operations lookup` UI now enables `Cancel shipment` only from backend action posture/readiness;
- this change does not implement webhooks, retry scheduling/queue, background worker fleet, broad React admin shipment dashboard/table, storefront/store shipment operations, checkout rewrite, new provider support or new required env; next implementation items remain live retry redispatch contract validation/materialization, webhook/post-v1 lifecycle work, live Yandex cancel endpoint validation, optional ledger/event transition extension and any broader shipment dashboard/table/filter/export only after separate approval.
- current tranche additionally materializes a manual-only provider-contract validation preparation contour for step 7: runbook [`delivery_hub_yandex_provider_contract_validation_runbook.md`](./delivery_hub_yandex_provider_contract_validation_runbook.md), harness [`delivery-hub-yandex-provider-contract-validation.ts`](../medusa-agency-boilerplate/src/scripts/delivery-hub-yandex-provider-contract-validation.ts), and redaction/evidence helper [`provider-contract-validation-evidence.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/provider-contract-validation-evidence.ts). Harness semantics are fail-closed and safe-by-default (`plan` + `dry_run=true`, explicit live opt-in env + confirm required, blocked when encryption key/connection readiness are missing), and output is structured redacted evidence.
- `2026-04-24` Tranche B controlled validation run is now documented by evidence bundle [`delivery_hub_yandex_provider_contract_validation_evidence_20260424.md`](./delivery_hub_yandex_provider_contract_validation_evidence_20260424.md): runbook operation matrix was executed in plan/dry-run, live preflight gates were explicitly exercised, and live provider calls remained not executed (`live_call_performed=false`) due fail-closed blockers in the environment (`blocked_live_opt_in_required`, `blocked_live_confirm_required`, `blocked_encryption_key_required`, `blocked_connection_not_found`). Therefore authoritative live provider semantics for create/status/cancel/repeated-cancel/missing-invalid reference are still unproven and remain pending a prerequisite-ready manual live session.

- current tranche additionally materializes §16.17 and §16.17.1 step 5 as manual retry policy/path v1 (readiness-first, blocked-only execution): new helper [`shipment-retry-policy.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/shipment-retry-policy.ts) resolves strict retry readiness against lifecycle/ledger/idempotency/diagnostic/duplicate-prevention gates and returns truthful blocked-only retry result when execution is requested, service path [`DeliveryHubService.retryAdminShipmentOperationsExecution()`](../medusa-agency-boilerplate/src/modules/delivery-hub/service.ts:469) plus route [`POST /admin/delivery/shipments/:execution_reference/operations/retry`](../medusa-agency-boilerplate/src/api/admin/delivery/shipments/[execution_reference]/operations/retry/route.ts) wire admin-only retry request handling through route-level sanitizer boundary [`sanitizeAdminDeliveryAdminShipmentOperationsRetryResponse()`](../medusa-agency-boilerplate/src/api/admin/delivery/shared.ts:1701), middleware adds explicit admin auth on the retry endpoint in [`middlewares.ts`](../medusa-agency-boilerplate/src/api/middlewares.ts:85), admin operations snapshot exposes retry readiness/action posture via [`admin-shipment-operations.ts`](../medusa-agency-boilerplate/src/modules/delivery-hub/admin-shipment-operations.ts), and React Admin shipment operations lookup adds retry endpoint builder + guarded button through [`page-state.ts`](../medusa-agency-boilerplate/src/admin/routes/settings/delivery/page-state.ts:1363) and [`page.tsx`](../medusa-agency-boilerplate/src/admin/routes/settings/delivery/page.tsx:815); focused no-network coverage in [`delivery-hub-shipment-retry-policy.unit.spec.ts`](../medusa-agency-boilerplate/src/workflows/__tests__/delivery-hub-shipment-retry-policy.unit.spec.ts), [`delivery-hub-admin-shipment-operations.unit.spec.ts`](../medusa-agency-boilerplate/src/workflows/__tests__/delivery-hub-admin-shipment-operations.unit.spec.ts) and [`page-state.unit.spec.ts`](../medusa-agency-boilerplate/src/admin/routes/settings/delivery/__tests__/page-state.unit.spec.ts) now locks allowed/blocked readiness matrix, duplicate/drift/ledger blockers, endpoint URL encoding, middleware admin auth registration, UI disabled posture and anti-leak assertions.

### 16.20. Pre-cutover next steps (scoped)

Следующие шаги между текущим состоянием (19 materialized read-only shadow preview layers + pre-cutin neutral selection rehearsal) и началом реального checkout cutover. Этот список **не является обещанием немедленного cutover** и **не создаёт новых backend commitments**.
