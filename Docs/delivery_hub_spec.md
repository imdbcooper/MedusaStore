...
`Auto confirm`, `Label generation type` и `Default warehouse id` уже materialized в UI этой tranche, но пока остаются thin config fields: это не означает, что warehouse CRUD, label pipeline или shipment lifecycle уже реализованы.
...
- store routes `quotes/points/pickup-windows`
...
- `listWarehouses` — reserved (warehouse domain в v1 ещё не материализован, см. §16.4).
...
| `delivery_warehouse` | deferred | этап B/E |
...
Store API не реализован в v1 и явно отнесён к этапу D (storefront cut-in). До этого storefront продолжает использовать legacy `apiship` surface.
...
- **Этап B (Admin foundation)** — частично. Реализовано: materialized `Settings -> Delivery`, `Providers` list, `Connections` CRUD, `Yandex connection` form, `Test connection`, `Connection diagnostics`. Не реализовано: warehouses CRUD (см. §16.5), `/connections/:id/disable` как отдельный роут, logs UI.
- **Этап C (Yandex quote engine)** — частично. Реализовано: `warehouse_to_pickup_point`, `dropoff_point_to_pickup_point`, listPickupPoints, listPickupWindows, admin `Test Quote` route/form. Не реализовано: `/admin/delivery/logs` UI, debug summary-экран, integration tests на Yandex test mode.
...
- Текущий tranche-1 admin UI = `Providers` (read-only), `Connections` (list + create/edit + test), `Yandex connection`, `Connection diagnostics`, `Test Quote` (ad-hoc форма). Всё.
- `Warehouses`, `Delivery Modes`, `Logs` — этап C/E.
...
- `Auto confirm`, `Label generation type`, `Default warehouse id` в tranche-1 работают как config passthrough fields и не должны читаться как доказательство materialized warehouse/shipment maturity.

`default_warehouse_id` в `DeliveryHubConnectionConfigSchema` в v1 означает **локальный** `delivery_warehouse.id` (после того, как warehouse domain появится). До этого поле остаётся опциональным и не используется.
