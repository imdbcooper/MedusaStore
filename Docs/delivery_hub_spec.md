...
- **Этап E (Shipment operations)** — current state is no longer purely preview-only/no-network on the runtime fulfillment path: the first real direct Yandex `create_shipment` runtime dispatch now exists, but only behind a strict gate and still without persistence/lifecycle activation. Pure helper [`materializeYandexCreateShipmentPayloadPreview()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-materializer.ts:191) по-прежнему детерминированно собирает redacted/safe payload preview из persisted backend-only handoff/provider-origin execution context для [`warehouse_to_pickup_point`](../medusa-agency-boilerplate/src/modules/delivery-hub/constants.ts:29) и [`dropoff_point_to_pickup_point`](../medusa-agency-boilerplate/src/modules/delivery-hub/constants.ts:30), returning `ready|blocked` with explicit blockers `missing_or_unsupported_mode`, `missing_provider_origin_dispatch_context`, `wrong_origin_context_for_mode`, `missing_destination_pickup_point`, `missing_pickup_interval_window`, `missing_recipient_contact`, `missing_package_items`; preview does not include credentials/auth headers/raw backend execution token/raw provider payload and masks provider/internal references. Narrow direct-Yandex dispatch-port contract [`buildYandexCreateShipmentDispatchPortContract()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:145) now truthfully reports blocked versus attempted runtime states. The same adapter-local file contains reusable executable request/result layer [`buildYandexCreateShipmentDispatchRequest()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:179) plus [`executeYandexCreateShipmentDispatch()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:211), and these primitives are now wired into [`buildDeliveryHubControlledFulfillmentExecutionResult()`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts:168) and runtime [`createFulfillment()`](../medusa-agency-boilerplate/src/modules/deliveryhub.ts:235). The live provider call is performed only when all strict prerequisites are simultaneously true: gate `DELIVERY_HUB_SHIPMENT_EXECUTION_ENABLED` is enabled, committed provider is Yandex, mode is supported, backend-only provider execution reference exists, provider-origin dispatch context matches the mode, payload materialization preview is `ready`, and the committed connection is active/enabled with sealed credentials. Earlier blocked contours still perform zero network calls. Runtime surface remains deliberately redacted and narrow: controlled execution can now reach `status=dispatch_attempted`, [`provider_payload_materialization`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts:98) and [`provider_dispatch_port`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts:97) stay summary-only, [`provider_dispatch_result`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts:137) returns only normalized safe success/failure categories, and [`dispatch_result`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts:138) explicitly confirms no shipment persistence, no execution-ledger persistence, no order mutation and no fulfillment mutation. Retries, compensation, webhooks, polling/status sync and broader shipment lifecycle remain not implemented in this tranche.
...
- factual boundary is now stricter and narrower than before: live provider invocation on the fulfillment path exists only under the exact gate + supported-mode + provider-origin-ready + materialization-ready + connection-ready contour; all blocked contours still perform zero network calls, and runtime logs/results stay redacted with no raw provider body, auth headers, execution token or quote key exposure;
- tests/status narrative переписан так, чтобы explicitly удерживать factual boundaries текущего shipment contour: direct Yandex runtime dispatch is now real but gated, while shipment persistence, execution-ledger activation/persistence, retries, compensation, webhook/polling lifecycle and order/fulfillment mutation remain disabled or not implemented in this tranche.
