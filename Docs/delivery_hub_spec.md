...
7. **Live Yandex validation matrix**
   - **Current status:** `manual-only harness/runbook materialized; live evidence collection pending`.
   ...
   - **Definition of Done/evidence:** sanitized run log/evidence package with environment labels only, expected/actual matrix, PASS/BLOCKED outcomes, no secret fragments and linked docs update. This tranche itself does not claim collected live evidence unless operator performs an explicit manual live session.
