...
- this change does not implement Yandex status polling, provider status API calls, webhooks, cancel, retry scheduling/queue, checkout rewrite, broad admin/store surface, new provider support or new required env; next implementation item remains §16.17.1 step 2 **Yandex status polling v1**.
