...
Current factual baseline: direct Yandex dispatch exists, accepted shipment persistence exists, execution-ledger activation exists, replay/duplicate/drift blockers are hardened, failed path remains failed-blocked, shipment persistence exists only for accepted dispatch outcome, accepted shipment lifecycle read-model exists, backend-only Yandex status polling v1 can refresh only accepted persisted shipments with redacted neutral status markers, a first admin/backend shipment-operations visibility slice can expose one sanitized execution-reference snapshot plus guarded refresh-status and cancel actions, and the existing `Settings -> Delivery` React Admin page now has a minimal single-reference `Shipment operations lookup` UI/page-state slice over those endpoints. Cancel v1 is backend/admin-only, accepted-Yandex-only, redacted, blocked before provider calls for unsafe states, and its direct Yandex adapter semantics remain local/mock-covered until live validation. This roadmap does not claim implemented webhooks, retry automation, broad React admin shipment dashboard/table, checkout rewrite or additional providers.
