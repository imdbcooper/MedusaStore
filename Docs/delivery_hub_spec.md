...
- **Этап E (Shipment operations)** — current state is still preview-only/no-network, but now includes three materialized backend-only slices. First, pure helper [`materializeYandexCreateShipmentPayloadPreview()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-materializer.ts:196) детерминированно собирает redacted/safe future Yandex `create_shipment` payload preview из уже persisted backend-only handoff/provider-origin execution context для [`warehouse_to_pickup_point`](../medusa-agency-boilerplate/src/modules/delivery-hub/constants.ts:30) и [`dropoff_point_to_pickup_point`](../medusa-agency-boilerplate/src/modules/delivery-hub/constants.ts:31). Он возвращает `ready|blocked` и explicit blockers `missing_or_unsupported_mode`, `missing_provider_origin_dispatch_context`, `wrong_origin_context_for_mode`, `missing_destination_pickup_point`, `missing_pickup_interval_window`, `missing_recipient_contact`, `missing_package_items`; preview не содержит credentials/auth headers/raw backend execution token/raw provider payload and masks provider/internal references. Second, narrow direct-Yandex dispatch-port contract [`buildYandexCreateShipmentDispatchPortContract()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:33) now materializes the runtime execution boundary for future direct `create_shipment` invocation without actually invoking the adapter: the contract is always explicit that `implemented=false`, `dispatch_attempted=false`, `dispatch_blocked=true`, and it truthfully distinguishes at least `execution_gate_disabled`, `dispatch_port_not_implemented`, `provider_not_supported` and `mode_not_supported`. Third, [`buildDeliveryHubControlledFulfillmentExecutionResult()`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts:468) wires both helpers into the final controlled [`createFulfillment()`](../medusa-agency-boilerplate/src/modules/deliveryhub.ts:235) seam and now emits both redacted `provider_payload_materialization` summary (`status`, `attempted`, `ready`, `blocked_reason_code`, `blocked_reason`, provider execution reference/context presence flags, redacted preview-summary counts/booleans and explicit anti-leak confirmations) and redacted `provider_dispatch_port` summary (`available`, `implemented`, `execution_gate_enabled`, `dispatch_attempted`, `dispatch_blocked`, `blocked_reason_code`, preview materialization readiness booleans and supported contour info). These helpers are invoked only after the seam has reached a meaningful provider-dispatch-contract stage; earlier blocked stages stay `not_attempted` for payload preview while dispatch-port summary remains a truthful blocked readiness contract. Even when payload preview is `ready` and the execution gate is enabled, runtime remains intentionally inert: no live Yandex adapter invocation, no provider network dispatch, no shipment persistence, no execution-ledger persistence activation, no order/fulfillment mutation and `live_adapter_call_performed=false`.
