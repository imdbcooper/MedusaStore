...
Current factual baseline: direct Yandex dispatch exists, accepted shipment persistence exists, execution-ledger activation exists, replay/duplicate/drift blockers are hardened, failed path remains failed-blocked, shipment persistence exists only for accepted dispatch outcome, accepted shipment lifecycle read-model exists, backend-only Yandex status polling v1 can refresh only accepted persisted shipments with redacted neutral status markers, a first admin/backend shipment-operations visibility slice can expose one sanitized execution-reference snapshot plus a guarded refresh-status action, and the existing `Settings -> Delivery` React Admin page now has a minimal single-reference `Shipment operations lookup` UI/page-state slice over those endpoints. This roadmap does not claim implemented webhooks, cancel, retry automation, broad React admin shipment dashboard/table, checkout rewrite or additional providers.
...
- this change does not implement webhooks, cancel, retry scheduling/queue, background worker fleet, broad React admin shipment dashboard/table, storefront/store shipment operations, checkout rewrite, new provider support or new required env; next implementation items remain cancel path policy, manual retry policy, webhook/post-v1 lifecycle work and any broader shipment dashboard/table/filter/export only after separate approval.
