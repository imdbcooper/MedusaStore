...
- **Этап E (Shipment operations)** — current state is still preview-only/no-network on the runtime fulfillment path, but now includes four materialized backend-only slices. First, pure helper [`materializeYandexCreateShipmentPayloadPreview()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-materializer.ts:191) детерминированно собирает redacted/safe future Yandex `create_shipment` payload preview из уже persisted backend-only handoff/provider-origin execution context для [`warehouse_to_pickup_point`](../medusa-agency-boilerplate/src/modules/delivery-hub/constants.ts:29) и [`dropoff_point_to_pickup_point`](../medusa-agency-boilerplate/src/modules/delivery-hub/constants.ts:30). Он возвращает `ready|blocked` и explicit blockers `missing_or_unsupported_mode`, `missing_provider_origin_dispatch_context`, `wrong_origin_context_for_mode`, `missing_destination_pickup_point`, `missing_pickup_interval_window`, `missing_recipient_contact`, `missing_package_items`; preview не содержит credentials/auth headers/raw backend execution token/raw provider payload and masks provider/internal references. Second, narrow direct-Yandex dispatch-port contract [`buildYandexCreateShipmentDispatchPortContract()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:145) now materializes the runtime execution boundary for future direct `create_shipment` invocation without actually invoking the adapter: the contract is always explicit that `implemented=false`, `dispatch_attempted=false`, `dispatch_blocked=true`, and it truthfully distinguishes at least `execution_gate_disabled`, `dispatch_port_not_implemented`, `provider_not_supported` and `mode_not_supported`. Third, the same adapter-local file now contains a reusable executable request/result layer for future direct dispatch: [`buildYandexCreateShipmentDispatchRequest()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:179) builds a real backend-only non-redacted provider request from the same validated inputs used by the preview materializer, [`executeYandexCreateShipmentDispatch()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:211) can call [`YandexDeliveryClient.post()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/client.ts:14) against adapter-local path [`YANDEX_CREATE_SHIPMENT_API_PATH`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:10), and the success/failure result surface is redacted/summary-only (`accepted`, `succeeded`, status category, masked/presence-only correlation and provider shipment references, label/doc presence booleans, explicit anti-leak flags). This executable layer is reusable from unit tests/future runtime wiring but is **not** yet invoked by [`createFulfillment()`](../medusa-agency-boilerplate/src/modules/deliveryhub.ts:119) and does **not** activate the live fulfillment path. Fourth, [`buildDeliveryHubControlledFulfillmentExecutionResult()`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts:468) still wires only the preview helper plus blocked dispatch-port readiness contract into the final controlled seam and now emits redacted `provider_payload_materialization` summary (`status`, `attempted`, `ready`, `blocked_reason_code`, `blocked_reason`, provider execution reference/context presence flags, redacted preview-summary counts/booleans and explicit anti-leak confirmations) and redacted `provider_dispatch_port` summary (`available`, `implemented`, `execution_gate_enabled`, `dispatch_attempted`, `dispatch_blocked`, `blocked_reason_code`, preview materialization readiness booleans and supported contour info). These runtime seam helpers are invoked only after the seam has reached a meaningful provider-dispatch-contract stage; earlier blocked stages stay `not_attempted` for payload preview while dispatch-port summary remains a truthful blocked readiness contract. Even when payload preview is `ready` and the execution gate is enabled, runtime remains intentionally inert: no live Yandex adapter invocation from [`createFulfillment()`](../medusa-agency-boilerplate/src/modules/deliveryhub.ts:119), no provider network dispatch on the fulfillment path, no shipment persistence, no execution-ledger persistence activation, no order/fulfillment mutation and `live_adapter_call_performed=false`.
...
- §16.17 `Этап E` синхронизирован с materialized backend-only wiring of [`materializeYandexCreateShipmentPayloadPreview()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-materializer.ts:191) plus narrow dispatch-port contract [`buildYandexCreateShipmentDispatchPortContract()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:145) into [`buildDeliveryHubControlledFulfillmentExecutionResult()`](../medusa-agency-boilerplate/src/modules/delivery-hub/fulfillment-execution-seam.ts), including preview-only `provider_payload_materialization` evidence, explicit `provider_dispatch_port` readiness contract and truthful gate-disabled versus gate-enabled-but-not-implemented boundary semantics;
- the same adapter-local shipment file now truthfully includes reusable executable dispatch primitives [`buildYandexCreateShipmentDispatchRequest()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:179) and [`executeYandexCreateShipmentDispatch()`](../medusa-agency-boilerplate/src/modules/delivery-hub/adapters/yandex/create-shipment-dispatch-port.ts:211) with redacted result normalization, but these are still **not** wired into [`createFulfillment()`](../medusa-agency-boilerplate/src/modules/deliveryhub.ts:119) or the runtime execution seam yet;
- tests/status narrative переписан так, чтобы explicitly удерживать factual boundaries текущего shipment contour: payload materialization readiness, dispatch-port readiness and reusable executable request/result normalization now exist for direct Yandex execution, but live adapter invocation on the fulfillment path, provider network dispatch from runtime wiring, shipment persistence, execution-ledger activation and order mutation remain disabled or not implemented.
