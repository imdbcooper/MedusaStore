...
3. **Minimal admin shipment operations panel**
   - **Current status:** `first backend/admin-only visibility slice materialized`; React page-state/widget integration and broad panel UX remain planned later.
...
- this change does not implement webhooks, cancel, retry scheduling/queue, background worker fleet, broad React admin shipment panel, storefront/store shipment operations, checkout rewrite, new provider support or new required env; next implementation items remain React admin page-state/widget integration for shipment operations if desired, cancel path policy, manual retry policy and webhook/post-v1 lifecycle work.
