...
def test_parse_markdown_normalizes_yaml_dates_in_metadata(tmp_path: Path):
    file_path = tmp_path / "knowledge.md"
    file_path.write_text(
        """---
...
""",
    ...
    assert chunks
    assert chunks[0].metadata["updated_at"] == "2026-05-11"
    assert chunks[0].metadata["audit"]["published_at"] == "2026-05-10"
