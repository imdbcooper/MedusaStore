...
`NEXT_PUBLIC_DELIVERY_HUB_CHECKOUT_CUTOVER_ENABLED=false` is reserved for a future approved checkout cutover and is parsed with explicit-true semantics only. The current runtime effect is read-only visibility/preflight status in the preview guardrails: false/absent shows default-off, true is recognized but still reports preflight/blocked-only with `canCommitShippingMethod=false`. It does not enable a Delivery Hub `setShippingMethod()` path.
