...
import sendOrderCanceledSmsNotificationWorkflow from "../workflows/send-order-canceled-sms-notification"

export default async function orderCanceledSmsNotificationHandler({
  event: { data },
  container,
  ...
  if (!orderId) {
    logger.warn(
      "[order-canceled-sms-notification] skip: order.canceled event received without order id"
    )

    return
  }

  const { result } = await sendOrderCanceledSmsNotificationWorkflow(container).run({
    input: {
      orderId,
    },
  })
