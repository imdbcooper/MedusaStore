...
import { enqueueAssistantReindexIntent } from "../lib/assistant-reindex-queue"
...
export async function enqueueAssistantProductReindex(
  ...
  void enqueueAssistantReindexIntent(args, intent).catch((error) => {
    logger.error(
      `[assistant-adapter] failed to enqueue intent ${intent.event_name} action=${intent.action} product_id=${productId} coalescing_key=${intent.coalescing_key} event_id=${intent.event_id ?? "n/a"}: ${error instanceof Error ? error.message : String(error)}`
    )
  })
}
...
export async function enqueueAssistantAllProductsReindex(
  ...
  void enqueueAssistantReindexIntent(args, intent).catch((error) => {
    logger.error(
      `[assistant-adapter] failed to enqueue broad reindex intent ${intent.event_name} scope=${intent.scope} reason=${intent.reason} coalescing_key=${intent.coalescing_key} event_id=${intent.event_id ?? "n/a"}: ${error instanceof Error ? error.message : String(error)}`
    )
  })
}
